
import { AUTH_ERROR_OCCURRED, MARKET_ERROR_OCCURRED, PURCHASED_ERROR_OCCURRED, USER_ERROR_OCCURRED } from '../constants/actions';

const authErrorsReducer = (state = [], action) => {
  switch (action.type) {
    case AUTH_ERROR_OCCURRED:
      return action.payload;
    default:
      return state;
  }
};

const marketErrorsReducer = (state = [], action) => {
  switch (action.type) {
    case MARKET_ERROR_OCCURRED:
      return action.payload;
    default:
      return state;
  }
};

const purchasedErrorsReducer = (state = [], action) => {
  switch (action.type) {
    case PURCHASED_ERROR_OCCURRED:
      return action.payload;
    default:
      return state;
  }
};


const userErrorsReducer = (state = [], action) => {
  switch (action.type) {
    case USER_ERROR_OCCURRED:
      return action.payload;
    default:
      return state;
  }
};


export { authErrorsReducer, marketErrorsReducer, purchasedErrorsReducer, userErrorsReducer };
